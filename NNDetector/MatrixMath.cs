...
namespace MatrixLibrary.Matrix
{
    ...
    public class MatrixMath
    {
        ...
        {
            Matrix resultMatrix = new Matrix(input.Rows, input.Cols);

            for(int i=0;i<input.Rows;i++)
            {
                for(int j=0;j<input.Cols;j++)
                {
                    resultMatrix[i, j] = 1.0 / (1.0 + Math.Exp((-1) * input[i, j]));
                }
            }
        ...
        public static Matrix SigmoidDerivative(Matrix input)
        {
            Matrix resultMatrix = new Matrix(input.Rows, input.Cols);
            ...
            for(int i=0;i<input.Rows;i++)
            {
                for(int j=0;j<input.Cols;j++)
                {
                    resultMatrix[i, j] = input[i, j] * (1.0 - input[i, j]);
                }
            }
        ...
        public static Matrix ElementWiseMultiplication(Matrix a,Matrix b)
        {
            if (a.Rows != b.Rows || a.Cols != b.Cols)
                throw new MatrixError("Can not do element wise multiplication!");
        ...
        public static Matrix Pow(Matrix input, int degree)
        {
            Matrix resultMatrix = new Matrix(input.Rows, input.Cols);
            ...
            for(int i=0;i<input.Rows;i++)
            {
                for(int j=0;j<input.Cols;j++)
                {
                    resultMatrix[i, j] = input[i, j] * input[i, j];
                }
            }
